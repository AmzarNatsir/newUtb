...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('hutang_kontainer', function (Blueprint $table) {
            $table->increments('id');
            $table->string('no_bayar', 50);
            $table->date('tgl_bayar');
            $table->integer('receive_id')->unsigned();
            $table->foreign('receive_id')->references('id')->on('receive_head');
            $table->integer('kontainer_id')->unsigned();
            $table->foreign('kontainer_id')->references('id')->on('common_kontainer');
    ...
    {
        Schema::dropIfExists('hutang_kontainer');
    }
};
