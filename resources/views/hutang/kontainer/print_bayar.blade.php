...
<head>
  ...
  <title>UTB | Print Bukti Pembayaran Hutang Kontainer</title>
  ...
  <tr>
      <td style="width: 50%; vertical-align: middle;">
        ...
        <tr class="trTop trLeft trRight">
            ...
            <td style="width: 2%;">:</td>
            <td style="width: 50%;">{{ $main->get_kontainer->nama_kontainer}}</td>
        </tr>
        <tr class="trLeft trRight">
            <td>Alamat</td>
            <td>:</td>
            <td>{{ $main->get_kontainer->alamat}}</td>
        </tr>
        <tr class="trLeft trRight">
            <td>Email</td>
            <td>:</td>
            <td>{{ $main->get_kontainer->email}}</td>
        </tr>
        <tr class="trButtom trLeft trRight">
            <td>No. Telepon</td>
            <td>:</td>
            <td>{{ $main->get_kontainer->no_telepon}}</td>
        </tr>
        </table>
      </td>
      <td style="width: 50%;">
...
<table style="width: 100%; font-size: 10pt; font-family: Arial, Helvetica, sans-serif;">
    ...
    <tr style="background-color: #808080; color:azure">
        ...
        <td style="width: 10%; text-align: center;">Invoice Kontainer</td>
        <td style="width: 10%; text-align: center;">Tgl. Tiba</td>
    ...
    <tr class="trButtom">
        ...
        <td style="text-align: center;">{{ $main->get_receive->invoice_kontainer }}</td>
        <td style="text-align: center;">{{ (!empty($main->get_receive->tgl_tiba)) ? date_format(date_create($main->get_receive->tgl_tiba), 'd-m-Y') : "" }}</td>
