<div class="modal-header">
    <h4 class="modal-title">Mutasi Pembayaran Hutang</h4>
...
<div class="modal-body">
    <div class="row">
        ...
        <thead>
            <tr>
                <th style="width: 20%;">Inv. Kontainer</th>
                <th style="width: 30%;">: {{ $main->invoice_kontainer }}</th>
                <th style="width: 20%;">Tgl. Tiba</th>
                <th style="width: 30%;">: {{ date_format(date_create($main->tgl_tiba), 'd-m-Y') }}</th>
            </tr>
            <tr>
                <th>Kontainer</th>
                <th>: {{ $main->get_kontainer->nama_kontainer }}</th>
                <th>Invoice</th>
                <th>: {{ $main->no_invoice }} - {{ date_format(date_create($main->tgl_invoice), 'd-m-Y') }}</th>
            </tr>
        </thead>
        </table>
        <table class="table table-striped">
            ...
            <tbody>
                <tr>
                    ...
                    <td style="text-align: right;"><b>{{ number_format($main->nilai_kontainer, 0) }}</b></td>
                    <td></td>
                </tr>
                @php
                    $nom=1;
                    $n_outs = $main->nilai_kontainer;
                @endphp
...
<script>
    ...
    {
        var id_data = $(el).val();
        window.open(route('pembayaranHutangKontainerMutasiPrint', id_data), "_blank");
    }
</script>