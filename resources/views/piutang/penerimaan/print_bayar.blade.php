...
<head>
  ...
  <title>UTB | Print Bukti Penerimaan Piutang</title>
...
<table style="width: 100%; font-size: 10pt; font-family: Arial, Helvetica, sans-serif;">
  <tr>
      <td style="width: 50%; vertical-align: middle;">
        ...
        <tr>
            <td colspan="3"><h5>BUKTI PENERIMAAN</h5></td>
        </tr>
        <tr class="trTop trLeft trRight">
            <td style="width: 30%;">DIterima Dari</td>
            <td style="width: 2%;">:</td>
            <td style="width: 50%;">{{ $main->get_penjualan->get_customer->nama_customer}}</td>
        </tr>
        <tr class="trLeft trRight">
            <td>Alamat</td>
            <td>:</td>
            <td>{{ $main->get_penjualan->get_customer->alamat}}</td>
        </tr>
        <tr class="trLeft trRight">
            <td>Kota</td>
            <td>:</td>
            <td>{{ $main->get_penjualan->get_customer->kota}}</td>
        </tr>
        <tr class="trButtom trLeft trRight">
            <td>No. Telepon</td>
            <td>:</td>
            <td>{{ $main->get_penjualan->get_customer->no_telepon}}</td>
        </tr>
        </table>
      </td>
      <td style="width: 50%;">
...
<table style="width: 100%; font-size: 10pt; font-family: Arial, Helvetica, sans-serif;">
    <tr>
        <td colspan="4" rowspan="2" style="height: 30px; vertical-align: middle;">Keterangan : {{ $main->keterangan }}</td>
    ...
    <tr style="background-color: #808080; color:azure">
        ...
        <td style="width: 10%; text-align: center;">Via</td>
        <td style="width: 10%; text-align: right;">Nominal&nbsp;</td>
    </tr>
    <tr class="trButtom">
        <td style="text-align: center; height: 40px;">{{ $main->get_penjualan->no_invoice }}</td>
        <td style="text-align: center;">{{ date_format(date_create($main->get_penjualan->tgl_transaksi), 'd-m-Y') }}</td>
        <td style="text-align: center;">{{ (!empty($main->get_penjualan->tgl_jatuh_tempo)) ? date_format(date_create($main->get_penjualan->tgl_jatuh_tempo), 'd-m-Y') : "" }}</td>
        <td style="text-align: center;">{{ ($main->metode_bayar==1) ? 'Tunai' : 'Transfer' }}</td>
        <td style="text-align: center;">{{ $main->get_via->penerimaan }}</td>
